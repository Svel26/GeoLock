...
public class PortalStitcher {
    ...
    public static boolean stitchOverworld(ServerLevel level) {
        ...
        try {
                ...
                for (int i = 0; i < wrappingZones.size(); i++) {
                    ...
                    if (wrappingZoneClass.isInstance(zoneObj)) {
                        ...
                        if (area != null) {
                            ...
                            // Check if zone covers the full world area
                            double expectedMin = -halfW;
                            double expectedMax = halfW - 1;
                            if (Math.abs(currentMinX - expectedMin) < 2.0 && Math.abs(currentMaxX - expectedMax) < 2.0 &&
                                ...
                                zoneMatches = true;
                                LOGGER.info("[GeoLock] Existing wrapping zone matches expected area.");
                            } else {
            ...
            if (!zoneMatches) {
                // Create zone that covers the full world area
                int xMin = (int) Math.floor(-halfW);
                int xMax = (int) Math.ceil(halfW - 1);
                int zMin = (int) Math.floor(-halfW);
                int zMax = (int) Math.ceil(halfW - 1);
                
                LOGGER.info("[GeoLock] Creating native inward wrapping zone at X range [{}, {}], Z range [{}, {}]",
                            xMin, xMax, zMin, zMax);
